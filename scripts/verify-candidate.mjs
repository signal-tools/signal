...
const expectedName = `signal-tools-signal-${expectedVersion}.tgz`;
...
if (metadata.name !== "@signal-tools/signal" || metadata.version !== expectedVersion) {
...
if (process.env.CANDIDATE_SHA !== undefined && manifest.sourceSha !== process.env.CANDIDATE_SHA) {
